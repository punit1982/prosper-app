        ...
        if analysis:
            ...
            if scores and isinstance(scores, dict):
                ...
                with bars_col:
                    ...
                    earn_g = _sf(info.get("earningsGrowth"))
                    if rev_g is not None:
                        _pct_rg = rev_g * 100 if abs(rev_g) < 1 else rev_g
                        _factor_context["revenue_growth"] = f"Rev: {_pct_rg:+.1f}%" + (f", Earn: {earn_g*100:+.1f}%" if earn_g is not None else "")
                    pm = _sf(info.get("profitMargins"))
                    om = _sf(info.get("operatingMargins"))
                    if pm is not None:
                        _factor_context["margins"] = f"Profit: {pm*100:.1f}%" + (f", Op: {om*100:.1f}%" if om is not None else "")
                    _de_v = _sf(info.get("debtToEquity"))
                    _cr_v = _sf(info.get("currentRatio"))
                    if _de_v is not None or _cr_v is not None:
                        _bs_parts = []
                        if _de_v is not None:
                            _bs_parts.append(f"D/E: {_de_v:.1f}")
                        if _cr_v is not None:
                            _bs_parts.append(f"CR: {_cr_v:.1f}")
                        _factor_context["balance_sheet"] = ", ".join(_bs_parts)
                    _pe_v = _sf(info.get("trailingPE"))
                    _fwd_pe_v = _sf(info.get("forwardPE"))
                    if _pe_v is not None:
                        _factor_context["valuation"] = f"P/E: {_pe_v:.1f}" + (f", Fwd: {_fwd_pe_v:.1f}" if _fwd_pe_v is not None else "")
                    _roe_v = _sf(info.get("returnOnEquity"))
            ...
            if fv and fv.get("base"):
                ...
                with fv_chart_col:
                    ...
                    fig_fv.add_trace(go.Scatter(
                        ...
                        text=[f"Bear<br>USD {_bear:,.0f}"], textposition="bottom center",
                        textfont=dict(size=10, color="#DD2C00"),
                        name=f"Bear (USD {_bear:,.2f})", showlegend=False,
                    ))
                    ...
                    fig_fv.add_trace(go.Scatter(
                        ...
                        text=[f"Base<br>USD {_base:,.0f}"], textposition="top center",
                        textfont=dict(size=11, color="#1a9e5c"),
                        name=f"Base (USD {_base:,.2f})", showlegend=False,
                    ))
                    ...
                    fig_fv.add_trace(go.Scatter(
                        ...
                        text=[f"Bull<br>USD {_bull:,.0f}"], textposition="bottom center",
                        textfont=dict(size=10, color="#00C853"),
                        name=f"Bull (USD {_bull:,.2f})", showlegend=False,
                    ))

                    # Current price line
                    if _current > 0:
                        fig_fv.add_vline(x=_current, line_dash="dash", line_color="white", line_width=2,
                                         annotation_text=f"Current: USD {_current:,.0f}",
                    ...
                    if _pw_fv > 0:
                        fig_fv.add_vline(x=_pw_fv, line_dash="dot", line_color="#f39c12", line_width=1.5,
                                         annotation_text=f"PW FV: USD {_pw_fv:,.0f}",
            ...
            with st.expander("Data Quality & Sources", expanded=False):
                ...
                with dq1:
                    ...
                    st.markdown(f"**Cost:** USD {_sf(cost):.4f}" if _sf(cost) is not None else "**Cost:** N/A")
                    if _analysis_date:
            ...
            with _exp_col:
                ...
                if fv and fv.get("base"):
                    ...
                    _export_lines.append(f"  Bear: USD {fv.get('bear', 0):,.2f} ({fv.get('prob_bear', '?')}% prob)")
                    _export_lines.append(f"  Base: USD {fv.get('base', 0):,.2f} ({fv.get('prob_base', '?')}% prob)")
                    _export_lines.append(f"  Bull: USD {fv.get('bull', 0):,.2f} ({fv.get('prob_bull', '?')}% prob)")
                    if _current and _current > 0:
                        ...
                        _export_lines.append(f"  Prob-Weighted FV: USD {_pw:,.2f} ({((_pw - _current)/_current*100):+.1f}% from current)")
                ...
                _export_lines.append(f"Tier: {tier_used} | Cost: USD {_sf(cost) or 0:.4f}")
                if data_sources:
