...
# ─────────────────────────────────────────
# Theme, base currency and refresh cadence are the controls anyone actually
# comes here to change. The other seven sections are plumbing — column toggles,
# API keys, source health, cache clearing, the About block — and they were all
# open at once, stacking into 4.5 phone screens of uninterrupted column with
# the theme switch at the top of it. They are behind disclosures now. Nothing
# was removed and no default changed.
...
# ─────────────────────────────────────────
with st.expander("📊 Dashboard preferences — which columns show on Holdings"):
...
# API STATUS
# ─────────────────────────────────────────
with st.expander("🌾 Options desk (HARVEST) rules"):
    st.caption(
...
with st.expander("🔑 API keys & integrations"):
    ...
    st.markdown(f"**" + "💡 How to add API keys" + "**")
    if True:  # was a nested expander — the section around it is already one
        st.markdown("""
...
# ABOUT
# ─────────────────────────────────────────
with st.expander("ℹ️ About Prosper, and the database it uses"):
    ...
    **Prosper** is an AI-native investment operating system for high-net-worth individuals
    ...
    try:
        if db_info.get("persistent"):
            st.markdown(f"**" + "☁️ Turso Connection Details" + "**")
            if True:  # was a nested expander — the section around it is already one
    ...
    if not db_info.get("persistent", False):
        st.markdown(f"**" + "☁️ Enable Cloud Database (Turso) — for persistent data" + "**")
        if True:  # was a nested expander — the section around it is already one
            st.markdown("""
